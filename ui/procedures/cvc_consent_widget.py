...
class CvcConsentWidget(QWidget):
    ...
    def collect(self, procedure_id: int = 0, doctor_name: str = "", diagnosis: str = "") -> ProcedureConsentDTO:
        consilium = {
            "doctor_1": self.consilium_1_edit.text().strip() if str(self.mode_combo.currentData() or "") == "consilium" else "",
            "doctor_2": self.consilium_2_edit.text().strip() if str(self.mode_combo.currentData() or "") == "consilium" else "",
            "doctor_3": self.consilium_3_edit.text().strip() if str(self.mode_combo.currentData() or "") == "consilium" else "",
