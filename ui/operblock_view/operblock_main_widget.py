...
class OperBlockMainWidget(QWidget):
    ...
    def auto_refresh(self, force: bool = False):
        ...
        elif self.stack.currentWidget() == self.archive_page:
    ...
    def _update_vitals_chart(self):
        if not self._current_admission_id or not self._current_operation_case_id or not getattr(self, "vitals_chart", None):
            return
        try:
            ...
            timeline_transform = OperBlockChartWidget.build_operation_timeline_transform(anesthesia_started_at, vitals)
            ...
            self.vitals_chart.admission_id = self._current_admission_id
            if hasattr(self.vitals_chart, "set_operation_timeline_model"):
                self.vitals_chart.set_operation_timeline_model(timeline_transform)
            self.vitals_chart.set_visible_hours(visible_hours)
            self.vitals_chart.update_data(vitals, display_start_dt, active_intervals=None)
            if hasattr(self.vitals_chart, "set_timeline_snapshot"):
                self.vitals_chart.set_timeline_snapshot(
                    getattr(self, "_current_timeline_snapshot", None),
                    display_start_dt,
                    force=True,
                )
            elif hasattr(self.vitals_chart, "set_operation_orders"):
                self.vitals_chart.set_operation_orders(getattr(self, "_current_orders_rows", []), display_start_dt, force=True)
    ...
    def _calculate_operblock_chart_hours(
        ...
        *,
        timeline_transform: _OperBlockTimelineTransform | None = None,
