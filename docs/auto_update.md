...
Updater показывает отдельное окно прогресса, ставит общий lock в `Baza_rao3_jurnal\locks\remcard_update.lock`, ждет закрытия активных окон врача/медсестры на этом же компьютере и заменяет управляемые файлы в рабочей папке `Prog`. Открытые окна РЕМКАРТА на других компьютерах не блокируют обновление текущего ПК.
